...
Все функции API работают асинхронно и могут принимать callback для получения ответа.

### Управление приложением

| Команда               | Описание                                           | Параметры                |
| --------------------- | -------------------------------------------------- | ------------------------ |
| `app.close()`         | Закрывает окно приложения.                         | -                        |
| `app.fullscreen()`    | Переключает окно приложения в полноэкранный режим. | -                        |
| `app.restore()`       | Восстанавливает обычный режим окна.                | -                        |
| `app.setTitle(title)` | Изменяет заголовок окна приложения.                | `title` — заголовок окна |

### Система

| Команда                   | Описание                                                                        | Параметры                                                |
| ------------------------- | ------------------------------------------------------------------------------- | -------------------------------------------------------- |
| `system.open(path, args)` | Открывает файл, папку, приложение или URL с помощью системной оболочки Windows. | `path` — путь или URL; `args` — необязательные аргументы |

### Работа с файлами

| Команда                      | Описание                       | Параметры                             |
| ---------------------------- | ------------------------------ | ------------------------------------- |
| `file.exists(path)`          | Проверяет существование файла. | `path` - путь к файлу                 |
| `file.writeText(path, text)` | Записывает текст в файл.       | `path` - путь к файлу; `text` - текст |
| `file.readText(path)`        | Читает текст из файла.         | `path` - путь к файлу                 |
| `file.delete(path)`          | Удаляет файл.                  | `path` - путь к файлу                 |

### Работа с папками

| Команда               | Описание                                     | Параметры             |
| --------------------- | -------------------------------------------- | --------------------- |
| `folder.list(path)`   | Возвращает список файлов и папок в каталоге. | `path` - путь к папке |
| `folder.exists(path)` | Проверяет существование папки.               | `path` - путь к папке |
| `folder.create(path)` | Создаёт папку.                               | `path` - путь к папке |
| `folder.delete(path)` | Удаляет пустую папку.                        | `path` - путь к папке |

### Буфер обмена

| Команда               | Описание                                           | Параметры      |
| --------------------- | -------------------------------------------------- | -------------- |
| `clipboard.get()`     | Возвращает текущий текст из буфера обмена Windows. | -              |
| `clipboard.set(text)` | Записывает текст в буфер обмена Windows.           | `text` - текст |

### Диалоги

| Команда                        | Описание                                               | Параметры                            |
| ------------------------------ | ------------------------------------------------------ | ------------------------------------ |
| `dialog.openFile(options)`     | Открывает стандартный диалог выбора файла Windows.     | `options` - дополнительные параметры |
| `dialog.saveFile(options)`     | Открывает стандартный диалог сохранения файла Windows. | `options` - дополнительные параметры |
| `dialog.selectFolder(options)` | Открывает стандартный диалог выбора папки Windows.     | `options` - дополнительные параметры |

### Callback

Любая функция API может принимать callback последним параметром:
